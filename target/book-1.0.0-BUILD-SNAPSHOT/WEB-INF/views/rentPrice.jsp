...
<div class="container">
	<table class="table table-bordered">
		...
		<section>
			<tr>
				<td colspan="5">
					<h2 style="text-align: center;">고객대여금액조회</h2>
					<%-- 고객목록조회 시작 --%>
					<div>
						<table class="table table-bordered">
							<tr>
							...
							<c:if test="${list eq null}">
								<tr>
									<td colspan="4">등록된 고객이 없습니다.</td>
								</tr>
							</c:if>
							<c:if test="${list ne null}">
								<c:forEach var="rentPrice" items="${list}">
									<tr>
										<td>${rentPrice.cust_no}</td>
										<td>${rentPrice.cust_name}</td>
										<td>${rentPrice.grade=="P" ? "Platinum" : (rentPrice.grade=="G" ? "Gold" : "Silver")}</td>
										<td>${rentPrice.sum_rent_price}</td>
									</tr>
								</c:forEach>
							</c:if>
						</table>
